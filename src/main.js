// CSSのインポートを削除（HTMLで読み込むため）
// import './style.css' // ←ここを消しました
...
async function startApp() {
    ...
    // --- ここから下は以前と同じですが、変数を調整しました ---
    let currentQuestions = [];
    let currentQuestionIndex = 0;
    let score = 0;
    const TOTAL_QUESTIONS = Math.min(10, questionsData.length);
    ...
    function showResult() {
        ...
        if (percentage === 100) resultMessage.textContent = 'パーフェクト！素晴らしい成績です💯';
        else if (percentage >= 80) resultMessage.textContent = '合格圏内です！その調子で頑張りましょう✨';
        else if (percentage >= 60) resultMessage.textContent = 'あと少し！復習を頑張りましょう📚';
        else resultMessage.textContent = 'もう少し勉強が必要かも？頑張って！💪';
    }
